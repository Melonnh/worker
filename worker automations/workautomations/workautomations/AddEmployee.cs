...
namespace workautomations
{
    ...
    {
        SqlConnection connect = new SqlConnection(@"Data Source=OMER\SQLEXPRESS;Initial Catalog=employeee;Integrated Security=True;");
       ...
       string.IsNullOrWhiteSpace(addEmployee_status.Text))
            ...
            if (check == DialogResult.Yes)
            {
                try
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(@"Data Source=OMER\SQLEXPRESS;Initial Catalog=employeee;Integrated Security=True;")) // connectionString'i doğru şekilde ayarladığınızdan emin olun
                    {
                        conn.Open();
